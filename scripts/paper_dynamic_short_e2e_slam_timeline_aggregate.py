...
def main():
    ...
    for index, host_path in enumerate(host_dirs):
        ...
        ax2.set_ylim(ymin=0, ymax=1.35)

        plt.gca().xaxis.set_major_locator(MaxNLocator(prune='lower'))
        plt.subplots_adjust(right=0.75)  # adjust the right margin here
        plt.scatter([], [], color='red', marker='+', label='SLAM uplink msg. size')
        plt.scatter([], [], color='orange', marker='+', label='SLAM downlink msg. size')
        plt.scatter([], [], color='green', marker='_', label='SLAM E2E Latency')
        ...
        filename = 'agg_SLAM_mobile_short_period.pdf'
