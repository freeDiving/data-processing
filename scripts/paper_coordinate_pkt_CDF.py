...
def main():
    ...
    for index, host_path in enumerate(host_dirs):
        ...
        filename = 'Coalesced_SLAM_pkt_size_vs_e2e_left_legend.pdf'
        ...
        host_to_cloud_pkt_list = get_coordinates_packets(drawing_moments=info_map["host"].pcap_drawing_moments,
                                                         mode="Host")
        cloud_to_resolver_pkt_list = get_coordinates_packets(drawing_moments=info_map["host"].pcap_drawing_moments,
                                                             mode="Resolver")

    # Draw
    host_to_cloud_pkt_list = np.sort(host_to_cloud_pkt_list)
    host_to_cloud_pkt_count, host_to_cloud_pkt_bins_count = np.histogram(host_to_cloud_pkt_list,
                                                                         bins=max(host_to_cloud_pkt_list))
    host_to_cloud_pkt_pdf = host_to_cloud_pkt_count / sum(host_to_cloud_pkt_count)
    host_to_cloud_pkt_cdf = np.cumsum(host_to_cloud_pkt_pdf)

    fig, ax = plt.subplots(figsize=(7.2, 5))
    ax.set_xlim(xmin=0, xmax=9000)
    ax.set_ylim(ymin=0, ymax=1)
    plt.gca().xaxis.set_major_locator(MaxNLocator(prune='lower'))
    host_to_cloud_pkt_bins_count = np.insert(host_to_cloud_pkt_bins_count[1:], 0, 0)
    #host_to_cloud_pkt_bins_count = [x / 1000 for x in host_to_cloud_pkt_bins_count]
    host_to_cloud_pkt_cdf = np.insert(host_to_cloud_pkt_cdf, 0, 0)
    plt.plot(host_to_cloud_pkt_bins_count, host_to_cloud_pkt_cdf,
             label="Host to Firebase", linewidth=3)

    cloud_to_resolver_pkt_list = np.sort(cloud_to_resolver_pkt_list)
    cloud_to_resolver_pkt_count, cloud_to_resolver_pkt_bins_count = np.histogram(cloud_to_resolver_pkt_list,
                                                                                 bins=max(cloud_to_resolver_pkt_list))
    cloud_to_resolver_pdf = cloud_to_resolver_pkt_count / sum(cloud_to_resolver_pkt_count)
    cloud_to_resolver_cdf = np.cumsum(cloud_to_resolver_pdf)
    cloud_to_resolver_pkt_bins_count = np.insert(cloud_to_resolver_pkt_bins_count[1:], 0, 0)
    cloud_to_resolver_cdf = np.insert(cloud_to_resolver_cdf, 0, 0)
    plt.plot(cloud_to_resolver_pkt_bins_count, cloud_to_resolver_cdf,
             label="Firebase to resolver", linewidth=3) # 10 is too thick
    ...
    plt.xlabel("Coordinate message size (bytes)", fontsize=my_font_size)
    plt.legend(loc="lower right", fontsize=my_font_size)

    plt.savefig(os.path.join(ROOT_PATH, f'../Figures/CDF_host_resolver_firebase_pkt_size.pdf'), format="pdf", bbox_inches="tight")
    plt.show()
