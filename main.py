...
def output_phases(timeline: List[Moment], output_path: str):
    ...
    with open(output_path, 'w') as f:
        ...
        for index, phase in enumerate(phases):
            for state in phase.output():
                f.write('{phase},{state},{duration},{start_time},{end_time}\n'.format(
                    phase='phase {}'.format(index + 1),
                    state=state,
                    duration=diff_sec(phase.get_state_start(state), phase.get_state_end(state)),
                    start_time=phase.get_state_start(state),
                    end_time=phase.get_state_end(state),
                ))
                if phase.is_last_state(state):
                    e2e_start_time = phase.get_e2e_start()
                    e2e_end_time = phase.get_e2e_end()
                    f.write('{phase},{state},{duration},{start_time},{end_time}\n'.format(
